...
for filename in all_files:
    ...
    # Check for colons
    if ":" in content:
        # ignore colons in YAML-like header or URLs, or markdown frontmatter markers
        lines = content.splitlines()
        for idx, line in enumerate(lines, 1):
            if ":" in line:
                if not (line.startswith("Created At") or line.startswith("Completed At") or line.startswith("File Path") or "://" in line or line.startswith("# ")):
    ...
    if "(" in content or ")" in content:
        ...
        for idx, line in enumerate(lines, 1):
            ...
            # allow standard acronyms like (HUD), (CoC), (CARE)
            clean_line = re.sub(r'\([A-Z]{3,4}\)', '', clean_line)
