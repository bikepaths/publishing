...
forbidden_patterns = [
    ...
    # intensity crutches (added for MoS_Survival_Physics)
    r"exact(?:ly)?", r"massive(?:ly)?", r"severe(?:ly)?", r"pure(?:ly)?",
    r"strict(?:ly)?", r"constant(?:ly)?", r"precise(?:ly)?",
]
...
for filename in all_files:
    ...
    # Colon check disabled — MoS_Survival_Physics authorizes colons for
    # formal lists and defining clauses. Manual review handles misuse.

    # Check for parentheses (allow in bibliography for Chicago 17th citations)
    is_bibliography = "bibliography" in filename.lower()
    if not is_bibliography and ("(" in content or ")" in content):
        lines = content.splitlines()
        for idx, line in enumerate(lines, 1):
            # clean out markdown links
            clean_line = re.sub(r'\[[^\]]*\]\([^)]+\)', '', line)
            # allow standard acronyms like (HUD), (CoC), (CARE), (SB 1338)
            clean_line = re.sub(r'\([A-Z]{2,5}(?:\s\d+)?\)', '', clean_line)
            if "(" in clean_line or ")" in clean_line:
                file_violations.append(f"Line {idx} contains parenthesis: {line.strip()}")

    # Check for forbidden words (skip bibliography)
    if not is_bibliography:
        for idx, line in enumerate(content.splitlines(), 1):
            matches = forbidden_regex.findall(line)
            if matches:
                file_violations.append(f"Line {idx} contains forbidden word(s) {set(matches)}: {line.strip()}")

    # Word count check (skip front matter, about author, bibliography)
    skip_wc = any(tag in filename for tag in ["front_matter", "about_the_author", "bibliography"])
    if not skip_wc:
        words = len(content.split())
        if words < 500:
            file_violations.append(f"Word count {words} is below minimum threshold of 500")
